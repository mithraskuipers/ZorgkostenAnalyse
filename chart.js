...
function initSliderBounds(g) {
  ...
  setSliderAttrs(
    ...
    xAbsMax,
    step(xAbsMax),
    Math.round(g.default_max),
  );
  setSliderAttrs(
    "y-min-sl",
    yAbsMin,
    yAbsMax,
    step(yAbsMax - yAbsMin),
    yAbsMin,
  );
  setSliderAttrs(
    "y-max-sl",
...
function resetAxis(axis_id) {
  if (axis_id === "x") {
    ...
    if (lastGraphData) {
      ...
      document.getElementById("x-max-sl").value = Math.round(
        lastGraphData.default_max,
      );
    }
    renderChart();
  } else {
...
function syncSlidersFromChart(chart) {
  ...
  const xMaxVal =
    labels[Math.min(labels.length - 1, hi)] ?? labels[labels.length - 1];

  const clamp = (v, a, b) => Math.max(a, Math.min(b, v));
  const xMinCl = clamp(
    Math.round(xMinVal),
    sliderBounds.xAbsMin,
    sliderBounds.xAbsMax,
  );
  const xMaxCl = clamp(
    Math.round(xMaxVal),
    sliderBounds.xAbsMin,
  ...
  const yMinCl = clamp(
    Math.round(yScale.min),
    sliderBounds.yAbsMin,
    sliderBounds.yAbsMax,
  );
  const yMaxCl = clamp(
    Math.round(yScale.max),
    sliderBounds.yAbsMin,
    sliderBounds.yAbsMax,
  );
